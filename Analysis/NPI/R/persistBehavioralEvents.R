...
#    basedir <- "/Volumes/eplab/Raw_Data/NV_Human/"
    ...
    for ( row in seq(1,nrow(data) ) ) {
      ...
      if ( data$code[row] >= 2 & data$code[row] <= 6 ) {
        dib$insert( list(subject=id, session=info$header$session_unique_ID, start=data$time[row], stop=data$time[row]+30E6, label=behaviorCodes[data$code[row]]) )
      }
    }
